...
strcpy(p, path);
    /*
     * INI : EM file input example
     */
char command[MAX_STR_CMD];
memset(command, 0, MAX_STR_CMD);
strcat(command, "cp /tmp/newTask.txt");
strcat(command, " ");
strcat(command, path);
printf("Intentando lanzar el comando (%s)\n", command);
printf("para copiar el fichero       (%s)\n", path);
if(system(command)==-1){
printf("Intentando lanzar el comando KO \n");
	return -1;
}
...
int return_create_work;
return_create_work = create_work(
        wu,
        in_template,
        path,
...
exit(retval);//Salida incondicional al generar un unico trabajo :-)
            ...
            log_messages.printf(MSG_CRITICAL,
                "em Work unit file (%s) ipath (%s) written\n", name, path
            );
		    log_messages.printf(MSG_DEBUG,
			"Job generated ret (%d) \n", retval
			);
