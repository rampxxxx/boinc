...
//********************************************
// Solo guardo un mensaje, para no llenar el disco
        //sprintf(buf, "%s: 0x%x\n", wu.name, wu.error_mask);
//        sprintf(buf, "El fichero %s: ha sido tratado y no guardado\n", wu.name);
    ...
    if (wu.canonical_resultid) {
        ...
        for (i=0; i<n; i++) {
            ...
            if (n==1) {
                ...
                copy_path = config.project_path("sample_results/%s.gz", wu.name);
            } else {
                //copy_path = config.project_path("sample_results/%s_%d", wu.name, i);
                copy_path = config.project_path("sample_results/%s_%d.gz", wu.name, i);
            }
